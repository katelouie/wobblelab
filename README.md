...
**Measuring *wobble*: how much a model's answer moves under things that shouldn't move it.**
...
did. WobbleLab is a small research toolkit for measuring that gap, and for reporting a
...
**Wobble is sensitivity to things that shouldn't matter.** A reliable answer stays put when
you poke it in ways that carry no meaning; a wobbly one slides around. There are exactly
...
Everything in WobbleLab is one of those two pokes, measured with a confidence interval, at
...
*Point wobble at a benchmark and you stop trusting the single number.* Three things fall
...
**2. Harness wobble: a 20-point spread from the eval convention alone.** Hold the items
...
![harness wobble](results/harness_world_religions.png)
...
**3. Config wobble: the sampling settings move it too** (a little). Swapping our neutral
...
## Lens B — prompt reliability (the wobble plane)
...
![the wobble plane](results/wobble_plane_v02.png)
...
- **COIN-FLIP** — wobbly on both axes.

The two axes collapse into a single **wobble score** when you need one number, with one
...
calibration, not wobble. So the score gates the observational term behind a decidability
...
**A benchmark number without its wobble is a claim without an error bar.** The entire
leaderboard genre reports point estimates for quantities that visibly move under choices
nobody standardizes. WobbleLab reports the number *and* how much it slides, which is the
...
src/wobblelab/
  ...
  wobble.py       the wobble score + decidability gate + model-level headline
...
experiments/
  bench.py        Lens A: position-debiased accuracy + CI + reorder wobble
  ...
  vpoc_real.py    Lens B: the wobble plane (dispersion x margin, typed paraphrases)
  score.py        Lens B: the wobble score, as a post-processing pass
...
python experiments/vpoc_real.py   # the wobble plane (Lens B)
...
```python
from wobblelab import wilson_ci, bootstrap_ci, wobble_score

wilson_ci(36, 100)                                   # a proportion, with an honest interval
wobble_score(dispersion=0.10, margin_max=0.47,       # cereal_soup: steady on re-run (low
...
>   accuracy±CI + wobble for two models on a benchmark. Most of the parts exist.
...
> - **Config-A/B on the benchmark**, where the wider output tail should make config wobble
>   bite the way it doesn't on binary prompts.
> - TruthfulQA MC1 (predict *higher* wobble, adversarial by construction); more subjects;
...
WobbleLab keeps a lab journal, not because the code needs it but because a reliability tool's
